					...
					<div style="width: 50%;" class="d-flex">
						<select name="typeSelect" class="form-select form-control me-2">
							...
							<option value="TCW" ${dto.types == 'TCW' ? 'selected' : ''}>제목
								OR 내용 OR 작성자</option>
						</select> <input type="text" class="form-control me-2" name="keywordInput"
					...
					<ul class="pagination">
						<c:if test="${dto.prev}">
							<li class="page-item"><a class="page-link"
								href="${dto.start -1}" tabindex="-1"> Previous </a></li>
						</c:if>
...
<script type="text/javascript" defer="defer">
  ...
  pagingDiv.addEventListener(
    ...
    (e) => {
      ...
      if(types && keyword){
	  // requestParam.set('types', types === '' ? null : types);
	   requestParam.set('types', types);
	  requestParam.set('keyword', keyword);
      }
      
      console.log(requestParam.toString());
      
      self.location = `/board/list?\${requestParam.toString()}`; // JavaScript 백틱, 템플릿
    },
    false
  );
  ...
  document.querySelector(".searchBtn").addEventListener(
    "click",
    () => {
      const _keyword = document.querySelector(
        "input[name='keywordInput']",
      ).value;
      const selectObj = document.querySelector("select[name='typeSelect']");
      const _types = selectObj.options[selectObj.selectedIndex].value;
