...
public class ProductMapperTests {
	...
	@Autowired
	public ProductMapperTests(ProductMapper productMapper) {
	...
	public void testUpdateOne() {
		ProductDTO productDto = ProductDTO.builder().pno(145).pname("Updated pname").pdesc("Updated pdesc").price(43210)
				.build();
	...
	public void testInsertDummies() {
		for (int i = 0; i < 45; i += 1) {
			ProductDTO productDto = ProductDTO.builder().pname("pName_" + i).pdesc("pDesc_" + i)
					.writer("user" + (i % 10)).price(3800).build();
